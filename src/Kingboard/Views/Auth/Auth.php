        ...
        if (isset($_POST["login"]) && isset($_POST["passwd"])) {
            if ($user = \Kingboard\Lib\Auth\Auth::login($_POST["login"], $_POST["passwd"])) {
                if ($user->status == \Kingboard\Model\User::STATUS_NEW) {
                    ...
                    return $this->render(
                        'user/activation_error.html',
                        array('activation_failed' => 'this user has not been activated yet, please check your mailbox for the activation email and follow its instructions.')
                    );
                }
    ...
    public function activateUser($request)
    {
        ...
        if ($user = \Kingboard\Model\User::findOne(array('validationCode' => $key))) {
            if ($user->status != \Kingboard\Model\User::STATUS_NEW) {
                return $this->render(
                    'user/activation_error.html',
                    array('activation_failed' => "no such activationkey")
                );
            }
        } else {
